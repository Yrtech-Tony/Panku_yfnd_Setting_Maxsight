    ...
    public partial class Subjects : BaseForm
    {
        ...
        private void SearchLink()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (ds.Tables[0].Rows[i]["Weight"] == DBNull.Value)
                    {
                        link.Weight = null;
                    }
                    else
                    {
                        link.Weight = Convert.ToDecimal(ds.Tables[0].Rows[i]["Weight"]);
                    }
        ...
        private void SearchSubject()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (ds.Tables[0].Rows[i]["Weight"] != DBNull.Value)
                    {
                        subject.Weight = Convert.ToDecimal(ds.Tables[0].Rows[i]["Weight"]);
                    }
                    else
                    {
                        subject.Weight = null;
                    }
                    //subject.FileNO = Convert.ToString(ds.Tables[0].Rows[i]["FileNO"]);
            ...
            if (CommonHandler.ShowMessage(MessageType.Confirm, "确定要保存吗？") == DialogResult.Yes)
            {
                ...
                foreach (SubjectDto subject in subjectList)
                {
                    ...
                    webService.SaveSubject(subject.StatusType, subject.ProjectCode, subject.SubjectCode, subject.Implementation,
                                           ...
                                           subject.ScoreCheck, subject.SubjectTypeCode, subject.SubjectTypeCodeExam, subject.SubjectDelChk, subject.AddErrorChk
                                           ,subject.Weight,subject.FileNO);
                }
            }
            SearchSubject();
        ...
        private void btnLinkSave_Click(object sender, EventArgs e)
        {
            ...
            if (CommonHandler.ShowMessage(MessageType.Confirm, "确定要保存吗？") == DialogResult.Yes)
            {
                ...
                foreach (LinkDto link in linkList)
                {
                    ...
                    link.CharterCode, link.LinkCode, link.LinkName, link.LinkContent, link.InUserID,link.Weight);
                }
            }
            SearchLink();
            CommonHandler.ShowMessage(MessageType.Information, "保存完毕");
        ...
        public override void SaveButtonClick()
        {
            ...
            if (CommonHandler.ShowMessage(MessageType.Confirm, "确定要保存吗？") == DialogResult.Yes)
            {
                ...
                {
                    webService.SaveProject(project.StatusType, project.ProjectCode, project.Year, project.Quarter);
                }
            }
            SearchProject();
            CommonHandler.ShowMessage(MessageType.Information, "保存完毕");
