        ...
        public override void SaveButtonClick()
        {
            ...
            if (CommonHandler.ShowMessage(MessageType.Confirm, "确定要保存吗？") == DialogResult.Yes)
            {
                ...
                foreach (ShopDto shop in shopList)
                {
                    ...
                    {
                        //webService.SaveShop(shop.SaleSmall, shop.AfterSmall, shop.ShopCode, shop.ShopName, shop.UseChk, shop.UserName,shop.Province,shop.City,shop.Password
                        //    ,shop.ShopNamePY,shop.ProvincePY,shop.CityPY,shop.Email);
                    }
        ...
        private void SearchShop()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    shop.ShopNamePY = Convert.ToString(ds.Tables[0].Rows[i]["ShopNamePY"]);
                    shop.ProvincePY = Convert.ToString(ds.Tables[0].Rows[i]["ProvincePY"]);
                    shop.CityPY = Convert.ToString(ds.Tables[0].Rows[i]["CityPY"]);
                    shop.Email = Convert.ToString(ds.Tables[0].Rows[i]["Email"]);
                    shopList.Add(shop);
                }
            }
