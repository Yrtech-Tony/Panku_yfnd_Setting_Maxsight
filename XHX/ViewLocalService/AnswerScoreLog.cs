...
namespace XHX.ViewLocalService
{
    public partial class AnswerScoreLog : BaseForm
    {
        ...
        private void SearchAnswerLog()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                {
                    ...
                    if (ds.Tables[0].Rows[j]["FirstscoreModifiDateTime"] == DBNull.Value)
                    {
                        answer.FirstscoreModifiDateTime = null;
                    }
                    else
                    {
                        answer.FirstscoreModifiDateTime = Convert.ToDateTime(ds.Tables[0].Rows[j]["FirstscoreModifiDateTime"]);
                    }
                    if (ds.Tables[0].Rows[j]["Firstscore"] == DBNull.Value)
                    {
                        answer.Firstscore = null;
                    }
                    else
                    {
                        answer.Firstscore = Convert.ToDecimal(ds.Tables[0].Rows[j]["Firstscore"]);
                    }
        ...
        {
            if ((e.Column == gcScore || e.Column == gcFirstRecheckScore || e.Column == gcSecondRecheckScore || e.Column == gcLastReCheckScore)
