...
namespace XHX.View
{
    ...
    public partial class AnswerSubjects : BaseForm
    {
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            ///*计算检查标准中选是，否，不涉及的个数*/
            //if (grvInspectionStandard.RowCount != 0)
            //{
            //    // 选是的个数
            //    int YesCount = 0;
            //    //选不涉及的个数
            ...
            //        }
            //    }
            //    NoCount = grvInspectionStandard.RowCount - notCheckCount - YesCount;
            ...
            //                if (NoCount == 0)
            //                {
            //                    CommonHandler.ShowMessage(MessageType.Information, "0分的情况下检查标准至少有一个选择：否");
            //                    return;
            //                }
            //            }
            //            else {
            //                if (NoCount != 0)
            //                {
            //                    CommonHandler.ShowMessage(MessageType.Information, "满分的情况下检查标准必须全部选择：是");
            //                    return;
            //                }
            //            }
            //        }
            ...
            //            if (Convert.ToDecimal(txtScore.Text) != fullScore)
            //            {
            //                if (NoCount == 0 && notCheckCount != (grcInspectionStandard.DataSource as List<InspectionStandardDto>).Count)
            //                {
            //                    CommonHandler.ShowMessage(MessageType.Information, "0分的情况下检查标准至少有一个选择：否");
            //                    return;
            //                }
            ...
            service.SaveAnswer(ProjectCode_Golbal, SubjectCode_Golbal, ShopCode_Golbal,
                            ...
                            CheckType, passReCheck, DateTime.Now.ToString("yyyy-MM-dd"), DateTime.Now.ToString("yyyy-MM-dd"), txtSpCode.Text, txtSpType.Text);
        ...
        private void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            //查询上一个问卷信息并显示
            if (GetSpecCase("", "", SubjectCode_Golbal).Count > 0)
            {
                btnSpecialCaseSearch.ForeColor = Color.Red;
            }
        ...
        private void Transfor()
        {
            ...
            GetFileAndPic();
            GetLossDesc(ProjectCode_Golbal, ShopCode_Golbal, SubjectCode_Golbal);
            /*确认这道题是否申请过特殊案例，如果申请过“特殊案例查询”按钮显示红色*/
            if (GetSpecCase("", "", SubjectCode_Golbal).Count > 0)
            {
                btnSpecialCaseSearch.ForeColor = Color.Red;
            }
        ...
        public bool GetSubject(DataSet ds, ButtonType buttonType)
        {
            ...
            {
                btnSpecialCaseSearch.ForeColor = Color.Black;
            }
            return true;
        }
        ...
        private void btnUploadDataToServer_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //Makefolder((CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text), "ftp://106.120.243.177/UpLoadImage/", "dsat", "dsat2011");
                    ...
                    //    DeleteFTPFile(url,
                    //        ("ftp://106.120.243.177/UpLoadImage" + "/" + (CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text+"/")).Replace("\\", "//"), "dsat", "dsat2011");
                    //}
                    foreach (string name in AllFilesList)
                    {
                        if (!name.Contains("Thumbs.db"))
                        {
                            ...
                            string url = temp[temp.Length - 1];
                            //DeleteFTPFile(url,
                            //    ("ftp://106.120.243.177/UpLoadImage" + "/" +
                            //    (CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text+@"/")).Replace("\\", "//"), "dsat", "dsat2011");
                            //Upload(name,
                            //    ("ftp://106.120.243.177/UpLoadImage" + "/" + (CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text) + "/" + url).Replace("\\", "//"), "dsat", "dsat2011");
