    ...
    public partial class ShopVinList : BaseForm
    {
        ...
        public override void SaveButtonClick()
        {
            ...
            if (CommonHandler.ShowMessage(MessageType.Confirm, "确定要保存吗？") == DialogResult.Yes)
            {
                ...
                foreach (ShopVinListDto shop in shopList)
                {
                    ...
                    {
                        webService.ShopVinListSave(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString(),shop.VinCode,shop.ShopCode,shop.ModelName
                            ,shop.SubModelName,shop.StockAge,shop.SaleFlag,shop.GradeNameCn,shop.ExteriorColor,shop.InteriorColor);
                    }
        ...
        private void SearchShop()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    shop.ModelName = Convert.ToString(ds.Tables[0].Rows[i]["ModelName"]);
                    shop.SubModelName = Convert.ToString(ds.Tables[0].Rows[i]["SubModelName"]);
                    shop.StockAge = Convert.ToString(ds.Tables[0].Rows[i]["StockAge"]);
                    shop.AreaCode = Convert.ToString(ds.Tables[0].Rows[i]["AreaCode"]);
                    shop.SaleFlag = Convert.ToString(ds.Tables[0].Rows[i]["SaleFlag"]);
                    shopList.Add(shop);
                }
            }
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Worksheet worksheet_FengMian = workbook.Worksheets["经销商库存表"] as Worksheet;

                //for (int i = 2; i < 26000; i++)
                //{
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "A"+i.ToString(), i.ToString());
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "B" + i.ToString(), i.ToString());
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "C" + i.ToString(), i.ToString());
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "D" + i.ToString(), i.ToString());
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "E" + i.ToString(), i.ToString());
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "F" + i.ToString(), i.ToString());
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "G" + i.ToString(), i.ToString());
                //    msExcelUtil.SetCellValue(worksheet_FengMian, "H" + i.ToString(), i.ToString());
                ...
                for (int i = 7; i < 7700; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(shopCode))
                    {
                        ...
                        string vinCode = msExcelUtil.GetCellValue(worksheet_FengMian, "C", i);
                        string modelName = msExcelUtil.GetCellValue(worksheet_FengMian, "D", i);
                        string submodelName = msExcelUtil.GetCellValue(worksheet_FengMian, "E", i);
                        string stockAge = msExcelUtil.GetCellValue(worksheet_FengMian, "F", i);
                        string saleFlag = msExcelUtil.GetCellValue(worksheet_FengMian, "G", i);
                        webService.ShopVinListSave(projectCode, vinCode, shopCode, modelName, submodelName, stockAge, saleFlag, "", "", "");
                    }
                }
