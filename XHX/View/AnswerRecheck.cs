...
namespace XHX.View
{
    ...
    public partial class AnswerRecheck : BaseForm
    {
        ...
        public AnswerRecheck(string projectCode, string subjectCode, string shopCode, int order, char checkType,string examType)
        {
            InitializeComponent();
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            //}
            service.DeleteRecheckDtl(ProjectCode_Golbal, SubjectCode_Golbal, ShopCode_Golbal, reCheckTypeCode, "");
            foreach (string errorCode in txtRecheckErrorCode.Text.Split('$'))
            {
                service.SaveRecheckDtl(ProjectCode_Golbal, SubjectCode_Golbal, ShopCode_Golbal, reCheckTypeCode, this.UserInfoDto.UserID, errorCode);
            }
            //查询下一个问卷信息并显示
        ...
        private void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            //查询上一个问卷信息并显示
            if (GetSpecCase(ProjectCode_Golbal, ShopCode_Golbal, SubjectCode_Golbal).Count > 0)
            {
                btnSpecase.Visible = true;
            }
            else
            {
                btnSpecase.Visible = false;
            }
        ...
        {
            //RecheckStatus();
            if (GetSpecCase(projectCode, shopCode, subjectCode).Count > 0)
            {
               btnSpecase.Visible = true;
            }
            else
        ...
        public bool GetSubject(DataSet ds, ButtonType buttonType)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                subjectDto.SpCode = Convert.ToString(ds.Tables[0].Rows[0]["SpCode"]);
                subjectDto.SpType = Convert.ToString(ds.Tables[0].Rows[0]["SpType"]);
            ...
            txtSpCode.Text = subjectDto.SpCode;
            txtSpType.Text = subjectDto.SpType;
            btnNext.Enabled = true;
            btnPrevious.Enabled = true;
            char checkType = '0';
        ...
        {
            string recheckErrorCode = string.Empty;
            string recheckErrorName = string.Empty;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    recheckErrorCode += Convert.ToString(ds.Tables[0].Rows[i]["ErrorType"])+"$";
                    recheckErrorName += Convert.ToString(ds.Tables[0].Rows[i]["RecheckErrorName"])+"$";
                    #region 注释
            ...
            txtRecheckErrorCode.Text = recheckErrorCode;
            txtRecheckErrorName.Text = recheckErrorName;
        }
        public List<SpecialCaseDto> GetSpecCase(string projectCode, string shopCode, string subjectCode)
        {
            List<SpecialCaseDto> spcialCaseList = new List<SpecialCaseDto>();
            DataSet ds = service.GetAllSpecialCase(projectCode, shopCode, subjectCode, new DateTime(2012, 1, 1), new DateTime(2020, 12, 31));
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    SpecialCaseDto speciCase = new SpecialCaseDto();
                    speciCase.ProjectCode = Convert.ToString(ds.Tables[0].Rows[i]["ProjectCode"]);
                    speciCase.SubjectCode = Convert.ToString(ds.Tables[0].Rows[i]["SubjectCode"]);
                    speciCase.ShopCode = Convert.ToString(ds.Tables[0].Rows[i]["ShopCode"]);

                    spcialCaseList.Add(speciCase);
                }
            }
            return spcialCaseList;
        }
        ...
        private void btnSpecase_Click(object sender, EventArgs e)
        {
            SpecialCaseSearchPop spop = new SpecialCaseSearchPop(ProjectCode_Golbal, ShopCode_Golbal, "", txtSubjectCode.Text, this.UserInfoDto);
            spop.ShowDialog();
        }

        private void simpleButton2_Click(object sender, EventArgs e)
        {
            RecheckErrorPopup error = new RecheckErrorPopup(txtRecheckErrorCode.Text);
            error.ShowDialog();
            List<RecheckErrorDto> list = error.linkList;

            string recheckErrorCode = string.Empty;
            string recheckErrorName = string.Empty;
            foreach (RecheckErrorDto errordto in list)
            {
                recheckErrorCode += errordto.ErrorTypeCode + "$";
                recheckErrorName += errordto.ErrorTypeName + "$";
            }
            txtRecheckErrorCode.Text = recheckErrorCode;
            txtRecheckErrorName.Text = recheckErrorName;
        }
