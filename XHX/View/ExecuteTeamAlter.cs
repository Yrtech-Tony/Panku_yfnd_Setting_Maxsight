...
namespace XHX.View
{
    public partial class ExecuteTeamAlter : BaseForm
    {
            ...
            foreach (ExecuteTeamAlterDto item in dataHandler.DataList)
            {
                if (item.StatusType == 'U')
                {
                    ...
                    if (item.AgreeCheck != null)
                    {
                        ...
                        , agreeCheck, item.AgreeReason, item.NewScore, this.UserInfoDto.UserID);
                    }
                    else {
        ...
        private void btnView_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            //sub.ShowDialog();
            AnswerSubjectForm answer = new AnswerSubjectForm(projectCode, subjectCode, shopCode, shopName, order, '1', this.UserInfoDto, examType);
            answer.ShowDialog();
        }
        public string GetRecheckDtl(string projectCode, string shopCode, string subjectCode, string recheckType)
        {
           ...
           // string recheckErrorName = string.Empty;
            ...
            return recheckErrorCode;
        }
        private void btnRecheckErrorView_Click(object sender, EventArgs e)
        {
             string projectCode = grvExecuteTeamAlter.GetRowCellValue(grvExecuteTeamAlter.FocusedRowHandle, gcProjectCode).ToString();
            string subjectCode = grvExecuteTeamAlter.GetRowCellValue(grvExecuteTeamAlter.FocusedRowHandle, gcSubjectCode).ToString();
            string shopCode = grvExecuteTeamAlter.GetRowCellValue(grvExecuteTeamAlter.FocusedRowHandle,gcShopCode).ToString();
            string list = GetRecheckDtl(projectCode, shopCode, subjectCode, "");
            RecheckErrorPopup error = new RecheckErrorPopup(list);
            error.ShowDialog();
        }
    }
}
