...
namespace XHX.View
{
    public partial class Area : BaseForm
    {
        ...
        public override void SaveButtonClick()
        {
            ...
            if (CommonHandler.ShowMessage(MessageType.Confirm, "确定要保存吗？") == DialogResult.Yes)
            {
                ...
                foreach (AreaDto area in areaList)
                {
                    ...
                    {
                        webService.SaveArea(area.AreaCode, area.AreaName, area.UpperAreaCode, area.AreaTypeCode, base.UserInfoDto.UserID,area.AreaMng,area.Password,area.AreaNameEn,area.Email);
                    }
                }
            }
            SearchArea();
        ...
        {
            CommonHandler.ExcelExport(grvArea);
        }
        private void SearchArea()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    area.AreaNameEn = Convert.ToString(ds.Tables[0].Rows[i]["AreaNameEn"]);
                    area.Email = Convert.ToString(ds.Tables[0].Rows[i]["Email"]);
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    area.UpperAreaCode = Convert.ToString(ds.Tables[0].Rows[i]["UpperAreaCode"]);
                    area.AreaTypeCode = Convert.ToString(ds.Tables[0].Rows[i]["AreaTypeCode"]);
                    area.AreaMng = Convert.ToString(ds.Tables[0].Rows[i]["AreaMng"]);
                    area.Password = Convert.ToString(ds.Tables[0].Rows[i]["Password"]);
                    area.AreaNameEn = Convert.ToString(ds.Tables[0].Rows[i]["AreaNameEn"]);
                    area.Email = Convert.ToString(ds.Tables[0].Rows[i]["Email"]);
                    areaList.Add(area);
                }
        ...
        private void btnPassword_Click(object sender, EventArgs e)
        {
            ...
            {
                for (int i = 0; i < grvArea.RowCount; i++)
                {
                    Random r = new Random(i);
                    int value = r.Next(100000, 999999);
                    grvArea.SetRowCellValue(i, gcPassword, value.ToString());
                }
            }
        }
    }
