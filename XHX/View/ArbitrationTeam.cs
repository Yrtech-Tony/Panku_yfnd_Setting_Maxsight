...
namespace XHX.View
{
    public partial class ArbitrationTeam : BaseForm
    {
        ...
        private void btnViewPic_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(dto.ProjectCode)== null)
            {
                CommonHandler.ShowMessage(MessageType.Information, "请选择一个项目");
                return;
            }

            if (string.IsNullOrEmpty(dto.ShopCode))
            {
            ...
            char checkType = '0';
            string examType = "";
            DataSet ds = service.SearchSubjectBySubjectCodeAndProjectCode(dto.ProjectCode, dto.SubjectCode);
            ...
            DataSet dsCheckType = service.SearchPassReCheckBySubjectCodeAndShopCode(dto.ProjectCode, dto.SubjectCode, dto.ShopCode);
            AnswerSubjectForm a = new AnswerSubjectForm(dto.ProjectCode,
                dto.SubjectCode,
                dto.ShopCode,dto.ShopName, order, checkType, this.UserInfoDto, examType);
            a.ShowDialog();
