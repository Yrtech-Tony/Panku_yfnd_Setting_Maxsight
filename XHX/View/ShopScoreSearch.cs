...
namespace XHX.View
{
    public partial class ShopScoreSearch : BaseForm
    {
        ...
        {
            InitializeComponent();
            // webService.Url = "http://192.168.13.240/XHX.BMWServer/service.asmx";
        ...
        public void Search()
        {
            ...
            string userNmae = txtName.Text;
            DateTime startDate = dateStart.DateTime;
            DateTime endDate = dateEnd.DateTime;
            List<TwoLevelColumnInfo> columnInfoList = SearchHead(projectCode, shopCode, userNmae, startDate, endDate);
            List<ShopScoreBodyDto> dataList = SearchBodyData(projectCode, shopCode, lossCheck, recheck, all, userNmae, startDate, endDate);
            ...
            foreach (GridBand band in grvShopScore.Bands)
            {
                if (band != gridBand4
                    && band != gridBand3
                    && band != gridBand2
        ...
        public List<TwoLevelColumnInfo> SearchHead(string projectCode, string shopCode, string userName, DateTime startDate, DateTime endDate)
        {
            //string[] s = shopCode.Split(',');
            DataSet ds = webService.SearchHead(projectCode, shopCode, userName, startDate, endDate);
        ...
        public List<ShopScoreBodyDto> SearchBodyData(string projectCode, string shopCode, bool lossCheck, bool recheck, bool all, string userName, DateTime startDate, DateTime endDate)
        {
            DataSet ds = webService.SearchBodyData(projectCode, shopCode, lossCheck, recheck, all, userName, startDate, endDate);
        ...
        public List<ShopScoreInfoDto> SearchLeft(string projectCode)
        {
            ...
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        info.SpeCase = GetSpecCase(CommonHandler.GetComboBoxSelectedValue(cboProject).ToString(),
                            btnShopCode.Text, info.SubjectCode).Count == 0 ? "" : "有";
                    }
        ...
        public List<SpecialCaseDto> GetSpecCase(string projectCode, string shopCode, string subjectCode)
        {
            ...
            DataSet ds = webService.GetAllSpecialCase(projectCode, shopCode, subjectCode, new DateTime(2012, 1, 1), new DateTime(2020, 12, 31));
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
        ...
        private void grvShopScore_DoubleClick(object sender, EventArgs e)
        {
                ...
                grvShopScore.FocusedColumn == gcSubjectCode)
            {
                ...
                AnswerSubjectForm a = new AnswerSubjectForm(projectCode,
                    subjectCode,
                    shopCode, txtShopName.Text, order, checkType, this.UserInfoDto, examType);
                a.ShowDialog();
            }
        }
        ...
        private void repositoryItemButtonEdit1_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            CommonHandler.LoadView(this, "AnswerRecheck", title, projectCode, subjectCode, shopCode, order, checkType, examType);
        }
    }
}
